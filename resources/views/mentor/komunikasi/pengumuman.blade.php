...
<style>
    ...
    .announcement-page .panel-card { border:0; border-radius:8px; }
    .announcement-page .stat-card { cursor:pointer; transition:.2s ease; }
    .announcement-page .stat-card:hover,
    .announcement-page .stat-card.active { transform:translateY(-3px); box-shadow:0 12px 28px rgba(42,143,189,.18); }
    ...
    .announcement-page .announcement-list { max-height:420px; overflow:auto; }
    .announcement-page .detail-box,
    .announcement-page .info-row { border:1px solid #d7eaf2; border-radius:8px; padding:12px; background:#fff; margin-bottom:10px; }
    .announcement-page .action-group { display:flex; flex-wrap:wrap; gap:6px; min-width:470px; }
...
<div class="container-fluid announcement-page">
    ...
    <section class="announcement-banner p-4 mb-4">
        <div class="row align-items-center g-3">
            <div class="col-lg-8">
                ...
                <p class="mb-0">Sampaikan informasi resmi kepada peserta secara terstruktur, terpusat, dan terdokumentasi.</p>
            </div>
            <div class="col-lg-4 text-lg-end">
                <span class="badge bg-light text-dark fs-6 me-2" id="announcementBadge">Publikasi</span>
                <button class="btn btn-dark" type="button" id="addAnnouncementBannerButton"><i class="bi bi-plus-circle"></i> Buat Pengumuman</button>
            </div>
        </div>
    </section>
    ...
    <div class="alert alert-warning d-flex align-items-center gap-2 mb-4">
        ...
        <div>Daftar di bawah menampilkan pengumuman yang diterima dari admin dan yang Anda kirim ke peserta, langsung dari database.</div>
    </div>
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-md-6 col-xl">
            <div class="card stat-card" data-status-card="terjadwal">
                <div class="card-body d-flex justify-content-between align-items-center">
                    <div><p class="text-muted mb-1">Dijadwalkan</p><h3 class="mb-0" id="statScheduled">0</h3></div>
        ...
        <div class="col-md-6 col-xl">
            <div class="card stat-card">
                <div class="card-body d-flex justify-content-between align-items-center">
                    <div><p class="text-muted mb-1">Jumlah Dibaca</p><h3 class="mb-0" id="statRead">0</h3></div>
                    <span class="stat-icon bg-dark"><i class="bi bi-eye"></i></span>
                </div>
            </div>
        </div>
        <div class="col-md-6 col-xl">
            <div class="card stat-card">
                <div class="card-body d-flex justify-content-between align-items-center">
                    <div><p class="text-muted mb-1">Keterbacaan</p><h3 class="mb-0" id="statReadRate">0%</h3></div>
        ...
        <div class="card-body">
            <div class="row g-3 align-items-end">
                <div class="col-lg-3 col-md-6">
                    <label class="form-label" for="searchInput">Judul Pengumuman</label>
                ...
                <div class="col-lg-2 col-md-6">
                    <label class="form-label" for="categoryFilter">Sumber</label>
                ...
                <div class="col-lg-2 col-md-6">
                    ...
                    <select class="form-select" id="statusFilter">
                        ...
                        <option value="terjadwal">Dijadwalkan</option>
        ...
        <div class="col-xl-6">
            <div class="card table-card shadow-sm h-100">
                <div class="card-header bg-white d-flex justify-content-between align-items-center">
                    ...
                    <span class="badge bg-warning text-dark" id="receivedCount">0</span>
                </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        <label class="form-label" for="targetInput">Target Penerima</label>
                        <select class="form-select" id="targetInput">
                            <option value="Peserta">Peserta</option>
                        </select>
                    </div>
                    <div class="col-md-4">
...
<div class="modal fade" id="confirmModal" tabindex="-1" aria-labelledby="confirmModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body" id="confirmText">Publikasikan pengumuman?</div>
            <div class="modal-footer">
                <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Batal</button>
                <button type="button" class="btn btn-warning" id="confirmActionButton">Publikasikan</button>
            </div>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', () => {
    const receivedAnnouncements = @json($receivedAnnouncementData ?? []);
    const sentAnnouncements = @json($sentAnnouncementData ?? []);
    ...
    const selectedItems = () => ({
        received: received.find((item) => item.id === selectedId),
        sent: sent.find((item) => item.id === selectedId),
    });
    const updateStats = () => {
        ...
        document.getElementById('statToday').textContent = all.filter((item) => item.date === new Date().toISOString().slice(0, 10)).length;
    ...
    const renderReceived = () => {
        ...
        body.innerHTML = received.map((item, index) => `
            <tr>
                <td>${index + 1}</td>
    ...
    const renderSent = () => {
        ...
        `).join('');
        empty.classList.toggle('d-none', sent.length > 0);
    };
    const renderAll = () => {
        updateStats();
        renderReceived();
    ...
    const openForm = (item = null) => {
        ...
        document.getElementById('targetInput').value = 'Peserta';
    ...
    document.getElementById('addAnnouncementBannerButton').addEventListener('click', () => openForm(null));
    document.getElementById('addAnnouncementButton').addEventListener('click', () => openForm(null));
    document.getElementById('draftButton').addEventListener('click', () => {
        const payload = readForm('draft');
        pendingAction = async () => {
            const endpoint = editingId ? `{{ url('/mentor/komunikasi/pengumuman') }}/${editingId}` : `{{ url('/mentor/komunikasi/pengumuman') }}`;
        ...
        document.getElementById('confirmText').textContent = `Simpan draft pengumuman "${payload.title}"?`;
        confirmModal.show();
    });
    document.getElementById('saveAnnouncementButton').addEventListener('click', () => {
        ...
        document.getElementById('confirmText').textContent = `Publikasikan pengumuman "${payload.title}" ke peserta?`;
        confirmModal.show();
    });
    document.getElementById('receivedTableBody').addEventListener('click', async (event) => {
        ...
        if (button.dataset.action === 'delete') {
            pendingAction = async () => {
                await apiRequest(`{{ url('/mentor/komunikasi/pengumuman') }}/${item.id}`, 'DELETE');
                received = received.filter((row) => row.id !== item.id);
                if (selectedType === 'received' && selectedId === item.id) selectedId = received[0]?.id || sent[0]?.id || null;
                showToast('Pengumuman berhasil dihapus.');
            };
            document.getElementById('confirmText').textContent = `Hapus pengumuman "${item.title}" dari daftar diterima?`;
            confirmModal.show();
        }
    });
    document.getElementById('sentTableBody').addEventListener('click', async (event) => {
        ...
        if (button.dataset.action === 'delete') {
            pendingAction = async () => {
                ...
                if (selectedType === 'sent' && selectedId === item.id) selectedId = sent[0]?.id || received[0]?.id || null;
                showToast('Pengumuman berhasil dihapus.');
            };
            document.getElementById('confirmText').textContent = `Hapus pengumuman "${item.title}"?`;
            confirmModal.show();
        }
    });
    document.getElementById('confirmActionButton').addEventListener('click', async () => {
        ...
        try {
            ...
            showToast(error.message || 'Gagal memproses pengumuman.', false);
        }
    });
